...
```bash
git clone https://github.com/jhu-ip/cs220-summer26-public.git
```
3.	Type:
``` bash
...
```bash
cp ~/cs220-summer26-public/exercises/ex03b/hello_world.c .
```
...
1.	While you do not want to remove or even modify the `cs220-summer26-public` folder (that is, your copy of our public class repo), you will no longer need to keep the temporary directory named `temp` that we created today, or any of the files you put inside it.  To remove the `temp` directory, first navigate to your home directory (that is, the top-level folder in your account) by typing `cd ~` or simply `cd`.
