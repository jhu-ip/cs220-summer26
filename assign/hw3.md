...
<li>Due: Tue Jul 7 at 11pm (Baltimore time).</li>
</ul>
</div>
</div>
...
## Git log
<p>
In the assignments folder of your private repository, create a new subfolder named `hw3`.  Do your work in that subfolder and use `git add`, `git commit` and `git push` regularly to backup your work as you make progress!</p>
...
Create a `.zip` file named `hw3.zip` containing your source/header files, `Makefile`, `gitlog.txt`, and `README`. Do not include any `.txt` files  and never submit any executable or object files!
